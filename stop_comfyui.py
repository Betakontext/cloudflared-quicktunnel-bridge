...
Conventions mirror restart_comfyui.py / restart_bridge.py:
...
CLI:
  ...
  --flag-name        Override flag filename (default: 'stop.flag')
...
Typical use:
  python stop_comfyui.py --message "Apply maintenance window"
  -> uploads /bridge/comfy/stop.flag (or COMFY_FLAG_REMOTE_DIR/stop.flag)

Server-side expectation:
  Your cf_quicktunnel_writer.py (or a local watcher) polls COMFY_FLAG_WATCH_DIR for 'stop.flag'
  and runs a local stop command for ComfyUI upon detection.
"""
...
def main():
    ...
    flag_name = (args.flag_name or "stop.flag").strip() or "stop.flag"
